...
![2_manifest](../../static/img/single-tenant/2_manifest.png)
 ...
 if tiy scroll down you will see the above `Redirect Url` in the manifest file as well.
  
 ![3_manifest_oneapp](../../static/img/single-tenant/10_manifest_one_app.jpg) 
...
![3_add_scope](../../static/img/single-tenant/4_add_scope.png)
...
Allows the app to access the API as you.
```
![3_add_scope_props](../../static/img/single-tenant/5_add_scope_props.png)
...
![6_authentication_method](../../static/img/single-tenant/11_Authentication_Add_a_platform.png)  
  
 
 Now enter `http://localhost:8000/oauth2-redirect` as the `Redirect URLs` and click on Configure as shown below:
  
![4_authentication_method_congire_single_page_app](../../static/img/single-tenant/12_signle_tenant_redirect_url_configure.png) 
...
![5_api_permissions_finish](../../static/img/single-tenant/9_api_permissions_finish.png)

Now if you click on the manifest you should see the changes you have made as shown below:
  
 ![5_updated_manifest](../../static/img/single-tenant/13_updated_manifest.png) updated_manifest.png
  
#### Step 7 - Note down your application IDs
Go back to the `Overview`, found in the left menu.
