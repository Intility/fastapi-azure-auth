...
the users claim **does not contain** the `idp` key, or preferably by configuring Azure AD. If you're using Azure AD to limit
...
Code example on how to check for the `idp` key:
