...
class Settings(BaseSettings):
    ...
    AUTH_POLICY_NAME: str = Field(default='', env='AUTH_POLICY_NAME')
...
Import the `B2CMultiTenantAuthorizationCodeBearer` from `fastapi_azure_auth` and configure it:


```python {6,42-51} title="main.py"
import uvicorn
from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware
from pydantic import AnyHttpUrl, Field
...
azure_scheme = B2CMultiTenantAuthorizationCodeBearer(
    ...
    openid_config_url=f'https://{settings.TENANT_NAME}.b2clogin.com/{settings.TENANT_NAME}.onmicrosoft.com/{settings.AUTH_POLICY_NAME}/v2.0/.well-known/openid-configuration',
